...
public class LargeNGramModel implements BackoffLanguageModel {
    ...
    public static final String PROP_CLEAR_CACHES_AFTER_UTTERANCE =
        "clearCachesAfterUtterance";
     ...
     */
    @S4Boolean(defaultValue = false)
    public final static String PROP_APPLY_LANGUAGE_WEIGHT_AND_WIP =
    ...
    public void newProperties(PropertySheet ps) throws PropertyException {
        ...
        clearCacheAfterUtterance =
            ps.getBoolean(PROP_CLEAR_CACHES_AFTER_UTTERANCE);
        maxDepth = ps.getInt(LanguageModel.PROP_MAX_DEPTH);
        dictionary = (Dictionary) ps.getComponent(PROP_DICTIONARY);
        applyLanguageWeightAndWip =
            ps.getBoolean(PROP_APPLY_LANGUAGE_WEIGHT_AND_WIP);
    ...
    public void allocate() throws IOException {
        ...
        if (location.getProtocol() == null ||
            ...
            try {
                loader =
                    new BinaryLoader(new File(location.toURI()),
                                     format,
                                     applyLanguageWeightAndWip,
                                     languageWeight,
                                     wip,
                ...
                loader =
                    new BinaryLoader(new File(location.getPath()),
                                     format,
                                     applyLanguageWeightAndWip,
                                     languageWeight,
                                     wip,
            ...
            loader =
                new BinaryStreamLoader(location,
                                       format,
                                       applyLanguageWeightAndWip,
                                       languageWeight,
                                       wip,
        ...
        for (int i = 1; i <= loader.getMaxDepth(); i++) {
            loadedNGramBuffers[i - 1] =
                new HashMap<WordSequence, NGramBuffer>();
        ...
        ngramDepthCache =
            new LRUCache<WordSequence, ProbDepth>(ngramCacheSize);
        if (dictionary != null)
    ...
    private void clearCache() {
        ...
        logger.info("LM Cache Size: " + ngramDepthCache.size() + " Hits: " +
    ...
    public ProbDepth getProbDepth(WordSequence wordSequence) {
        ...
        if (logFile != null && probDepth != null)
            logFile.println(wordSequence.toString().replace("][", " ") +
                            " : " + Float.toString(probDepth.probability) +
                            " : "
                            + probDepth.depth);
    ...
    private NGramBuffer loadNGramBuffer(WordSequence ws) {
        ...
        if (orderBuffer == 2) {
            ...
            position =
                (loader.getNGramOffset(orderBuffer) + (firstCurrentNGramEntry *
                                                       ((loader.getMaxDepth() == orderBuffer) ? BYTES_PER_NMAXGRAM
                                                               : BYTES_PER_NGRAM) * loader
                        .getBytesPerField()));
            ...
            size =
                numberNGrams *
                        ((loader.getMaxDepth() == orderBuffer) ? BYTES_PER_NMAXGRAM
                                : BYTES_PER_NGRAM) * loader.getBytesPerField();
            position =
                loader.getNGramOffset(orderBuffer) +
    ...
    private int getFirstNGramEntry(NGramProbability nMinus1Gram,
                                   int firstNMinus1GramEntry,
                                   int n) {
        int firstNGramEntry =
            ngramSegmentTable[n - 1][(firstNMinus1GramEntry + nMinus1Gram
                    .getWhichFollower())
    ...
    public Set<String> getVocabulary() {
        Set<String> vocabulary =
            new HashSet<String>(Arrays.asList(loader.getWords()));
    ...
    private void buildSmearInfo() throws IOException {
        ...
        for (int i = 0; i < loadedBigramBuffers.length; i++) {
            ...
            for (int j = 0; j < bigram.getNumberNGrams(); j++) {
                ...
                NGramProbability bgProb = bigram.getNGramProbability(j);
                float logbgbackoff =
                    ngramBackoffTable[2][bgProb.getBackoffID()];
                ...
                if (trigram == null)
                    ...
                    for (int l = 0; l < trigram.getNumberNGrams(); l++) {
                        ...
                        bg_numerator +=
                            (tgprob * logtgprob
                                    - backofftgprob * logbackofftgprob) *
                                    logugprob;
    ...
    private void dumpProbs(double[] ugNumerator,
                           double[] ugDenominator,
                           int i,
                           int j,
                           float logugprob,
                           float logbgprob,
                           double ugprob,
                           double bgprob,
                           double backoffbgprob,
                           double logbackoffbgprob) {
