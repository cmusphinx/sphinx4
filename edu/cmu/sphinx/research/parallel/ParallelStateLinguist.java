 ...
 */
public class ParallelStateLinguist implements edu.cmu.sphinx.decoder.linguist.Linguist {

    private static final String PROP_PREFIX = 
	"edu.cmu.sphinx.research.parallel.ParallelStateLinguist.";
    ...
    /**
     * Initializes this ParallelStateLinguist, which means creating the 
     ...
     * @param model this is not used in the ParallelStateLinguist, since
	...
	System.out.println("ParallelStateLinguist: using " + models.length +
			   " acoustic models");
	...
	compileGrammar(grammar);
        System.out.println("ParallelStateLinguist total states: " + 
                           allStates.size());
    }
    ...
    /**
     * Returns the language model for this ParallelStateLinguist.
     *
	...
	edu.cmu.sphinx.decoder.linguist.UnitState unitState = new edu.cmu.sphinx.decoder.linguist.UnitState
	...
	edu.cmu.sphinx.decoder.linguist.SentenceHMMState lastState = unitState;
	...
	lastState = expandHMMStates(hmms, unitState);
	...
	for (int i = 0; i < hmms.length; i++) {
		...
		double selfTransitionScore = 
		    logMath.linearToLog(selfTransition.getProbability());
		// connect the next combining state to this HMMState
		attachState(lastState, firstHMMState, 
			    selfTransitionScore, 0.0, 0.0);
	    }
	...
	for (int i = 1; i <= hmms[0].getOrder(); i++) {
		...
		attachState(lastState, hmmStateState,
			    logMath.linearToLog(arc.getProbability()),
			    0.0, 0.0);
