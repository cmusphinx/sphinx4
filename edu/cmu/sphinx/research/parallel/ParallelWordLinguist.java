...
public class ParallelWordLinguist implements Linguist {
    ...
    private static final boolean PROP_ADD_SELF_LOOP_WORD_END_SIL_DEFAULT =
        true;
    ...
    /**
     * Initializes this ParallelWordLinguist, which means creating the 
     ...
     * @param model this is not used in the ParallelWordLinguist, since
	...
	System.out.println("ParallelWordLinguist: using " + models.length +
			   " acoustic models");
	...
	compileGrammar(grammar);
        System.out.println("ParallelWordLinguist total states: " + 
                           allStates.size());
    }
    ...
    /**
     * Returns the language model for this ParallelWordLinguist.
     *
        ...
        for (int a = 0; a < acousticModels.length; a++) {
            ...
            for (int i = 0; i < units.length; i++) {
                ...
                if (i == 0 || i == (units.length - 1)) {
                    ...
                    unitState = new ParallelUnitState
                        (state, model.getName(), i, units[i], 
                         tokenStackCapacity);
                } else {
                    ...
                    Unit unit = new Unit(units[i].getName(), 
                                         units[i].isFiller(),
                                         context);
                    unitState = new ParallelUnitState
                        (state, model.getName(), i, unit, tokenStackCapacity);
                }
	...
	ParallelUnitState unitState = new ParallelUnitState
	    (pronunciationState, acousticModel.getName(), which, 
             Unit.SILENCE, tokenStackCapacity);
        unitState.setColor(Color.GREEN);
