...
public class TokenStackImpl implements TokenStack {
    ...
    private edu.cmu.sphinx.decoder.search.Token[] stack;


    /**
     * Constructs a TokenStackImpl object with the given capacity.
     *
     * @param capacity the height of the stack
     */
    public TokenStackImpl(int capacity) {
        this.stack = new edu.cmu.sphinx.decoder.search.Token[capacity];
        clear();
    }
     ...
     */
    public edu.cmu.sphinx.decoder.search.Token add(edu.cmu.sphinx.decoder.search.Token newToken) {
        edu.cmu.sphinx.decoder.search.Token lowestToken = null;
        int lowestTokenIndex = -1;

        // first look for the lowest scoring token
        for (int i = 0; i < stack.length; i++) {
            edu.cmu.sphinx.decoder.search.Token token = stack[i];
	    if (token == null) {
		stack[i] = newToken;
		return null;
	    } else {
