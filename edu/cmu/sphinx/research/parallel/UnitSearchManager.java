 ...
 */
public class UnitSearchManager implements edu.cmu.sphinx.decoder.search.SearchManager {

    // The sphinx property prefix for all property names of this class.
    private static final String PROP_PREFIX =
	"edu.cmu.sphinx.research.parallel.UnitSearchManager.";
    ...
    private ScoreCombiner unitScoreCombiner;
    ...
    /**
     * Initializes this UnitSearchManager with the given context,
     * linguist, scorer, and pruner. Note that the given pruner is unused
     * in this UnitSearchManager, since we use the FeatureScorePruner
	...
	unitScoreCombiner = new FeatureScoreCombiner();
    ...
    private void calculateCombinedScore(CombineToken token) {
        unitScoreCombiner.combineScore(token);
    }
		...
		nextState.getBestToken().getFrameNumber() != nextFrameNumber;
            ...
            if (nextState.getColor() == Color.RED) {
                ...
                TokenStack tokenStack = 
                    ((ParallelHMMStateState)nextState).getTokenStack();
