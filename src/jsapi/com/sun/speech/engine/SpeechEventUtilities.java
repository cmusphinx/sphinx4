 ...
 * help applications conform with the Swing Event Thread model.  If these utilities were not used, then a GUI
 * application would have to implement Runnables to handle JSAPI events that result in updates to the GUI.
 */
public class SpeechEventUtilities {
     ...
     * SpeechAWTEvents.  It is lazily created along with systemEventQueue in postSpeechEvent.
     *
     * @see #postSpeechEvent
     */
     ...
     * global debugging flag. If a class calling postSpeechEvent wants to wait until the SpeechEvent has been
     * dispatched, it should call the postSpeechEvent method that has the waitUntilDispatched parameter.
     *
     * @see #postSpeechEvent
     */
    ...
    /**
     * Determine if the AWT event queue is running.  This method is one big hack, and we will be entering a bug against
     * AWT to provide us with a real method for determining if AWT is active or not.  The problem with asking AWT if it
     * is active right now is that it will activate it if it isn't already active.
     */
    ...
    /**
     * Post a JSAPI SpeechEvent.  This is to be used by multiple processes to synchronize SpeechEvents.  It currently
     * uses the AWT EventQueue as a means for doing this, which has the added benefit of causing all SpeechEvent
     * notification to be done from the event dispatch thread.  This is important because the Swing Thread Model
     * requires all interaction with Swing components to be done from the event dispatch thread.
     * <p/>
     * This method will immediately return once the event has been posted if the global waitUntilDispatched flag is set
     * to false. Otherwise, it will wait until the event has been dispatched before returning.
     *
     ...
     * requires all interaction with Swing components to be done from the event dispatch thread.
     * <p/>
     * This method will immediately return once the event has been posted if the waitUntilDispatched parameter is set to
     * false. Otherwise, it will wait until the event has been dispatched before returning.
     *
     * @param dispatcher          the dispatcher that will dispatch the event
     * @param event               the SpeechEvent to post
    ...
    /**
     * Inner class that defines SpeechAWTEvents.  These are created and posted to the AWT EventQueue by the
     * postSpeechEvent method.
     *
     * @see #postSpeechEvent
     */
    @SuppressWarnings("serial")
