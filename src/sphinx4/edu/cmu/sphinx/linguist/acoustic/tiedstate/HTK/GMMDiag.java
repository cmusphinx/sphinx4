...
/**
 * C'est un ObsProducer: outputs les loglikes de chaque gauss
 * @author cerisara
 *
 */
public class GMMDiag {
	...
	public int getNgauss() {return ngauss;}
	...
	public void setVar(int i, int j, float v) {
		// attention ! on stocke -1/(2var)        
		if (v<=0)
			// on ne fait pas une erreur, car on peut se servir du GMM juste pour stocker des valeurs et les recuperer plus tard...
			// TODO: bon c est pas tres propre, car on doit quand meme avoir var > 0 !
	...
	/**
	 * sauve dans un format proprietaire (texte)
	 * @param nom
	 */
	...
	public void load(String nom) {
		try {
			...
			s = fin.readLine();
			if (s != null) {
				// optionnel : on peut ajouter a la fin le nb de data sur lequel le GMM a ete appris                
	...
	public void computeLogLikes(float[] data) {
		...
		for (int gidx = 0; gidx < ngauss; gidx++) {
			...
			// on inclut la proba a priori de chaque gauss
			loglikes[gidx] = weights[gidx] + logDval1gauss;
		}
	}
	/**
	 * calcule le loglike pour la valeur courante de l'observation en entree.
	 * 
	...
	public int getWinningGauss() {
		int imax=0;
		for (int i=1;i<ngauss;i++) {
			if (loglikes[i]>loglikes[imax]) imax=i;
		}
		return imax;
	}
	...
	public boolean isEqual(GMMDiag g) {
		if (getNgauss()!=g.getNgauss()) return false;
		if (getNgauss()!=g.getNcoefs()) return false;
		for (int i=0;i<getNgauss();i++) {
			if (isDiff(getWeight(i),g.getWeight(i))) return false;
			for (int j=0;j<getNcoefs();j++) {
				if (isDiff(getMean(i,j),g.getMean(i,j))) return false;
				if (isDiff(getVar(i,j),g.getVar(i,j))) return false;
			}
		}
		return true;
	}
