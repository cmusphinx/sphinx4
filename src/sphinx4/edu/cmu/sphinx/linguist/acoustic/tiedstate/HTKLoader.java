...
/* 3ph selection algorithm:
1) Look for a trigram that matches exactly in terms of unit, unit 
context and word position. (Word positions can be one of 'begining', 
...
(if you are interested in the code, look at 
TiedStateAcousticModel.lookupNearestHMM)
*/

/**
 * 
 * Remark1: S4 does not use HMM tying: the HTK "tiedlist" is not loaded nor user for now
 * 
 ...
 * In this file, we have simply tied all 1phs to the first 3ph found with the same basephone. 
 * This is clearly not the best option, this may impact the performances.
 * A better alternative should be proposed.
 * 
 * Remark3: HTK is case-sensitive, while S4 requires upper-case names. A good option would be
 * to modify S4 to make it case-sensitive, but in this version, I have rather built a class
 * "NamesConversion" to transform the models, lexicons and grammar.
 * 
 * Remark4: HTK does not have separate filler file. It should be quite easy to define the
 * fillers in the config file of HTKLoader, but I have not yet done it. For now, a separate filler
 * file must be built.
 * 
 * Remark5: The HTK/julius lexicon has different fields for the word name and the printed output.
 * We simply delete and don't use this field ("[...]") in this version. This modify the way scoring
 * must be done. WARNING !
 * 
 * Remark6: Diphones are not allowed in S4 (?), and when they occur in the HTK MMF,
 * 	 the 2ph is transformed into a 3ph with "SIL" context.
 * 	 When both the transformed 2ph and the 3ph coexists in the MMF file, only
 *   the first one is kept (!)
 * 
 * Remark7: HTKloader does not support yet all the HTK format. However it should work for standard
 *   3ph models (it supports state/transition tying).
 * 
 */
public class HTKLoader implements Loader {
     ...
     *
     * @param MMFname the name of the acoustic model; if null we just load from the default location
     */
    private void loadModelFiles(String MMFname) throws FileNotFoundException,
        ...
        // senonePool represente l'ensemble des etats emetteurs
     ...
     *
     * xtof: assumes the means and variances are in the same order
     * idem pour les weights: dans le meme ordre !
     * 
    ...
    private Pool<Senone> createSenonePool(float distFloor, float varianceFloor) {
        ...
        // TODO: supprimer cette variable pour autoriser un nb d'etats different par HMM
     ...
     */
    protected void loadHMMPool(boolean useCDUnits, HTKstruct htkmods, String path)
            throws IOException {
    	int numStatePerHMM;
    	...
    	/**
    	 * This version loads the 1ph from the .MMF
    	 * But contrary to S4, HTK do not produce reliable 1ph, and some 1ph may be missing !
    	 * An option might be to load 1ph trained independently from the 3ph
    	 * but this file may not be always available.
    	 * An easy fallback solution for now is to tye all the 1ph to the first
    	 * 3ph with the same basename found...
    	 */
    	if (!tie1ph) {
    		for (Iterator<SingleHMM> monoPhones = htkmods.hmmsHTK.get1phIt(); monoPhones.hasNext();) {
    			SingleHMM hmm = monoPhones.next();
    			if (hmm==null) break;
    			String name = hmm.getNom();
    			String attribute;
    			// TODO: this is not general enough !
    			// TODO: read the fillers from a file or a prop
    			if (name.equals("sil")||name.equals("sp")||name.equals("bb")||name.equals("xx")||name.equals("hh"))
    				attribute = FILLER;
    			else
    				attribute = "nofiller";
    			...
    			// stid doit contenir l'identifier du senone, tel que utilise
    			// lors de la creation du senonePool !!
    			int j=0;
    			for (int ii=0; ii<numStatePerHMM; ii++) {
    				if (hmm.isEmitting(ii)) {
    					HMMState s = hmm.getState(ii);
    					// on recupere l'index du GMM dans le fichier MMF
    					stid[j] = htkmods.hmmsHTK.getStateIdx(s); j++;
        ...
        for (Iterator<SingleHMM> triPhones = htkmods.hmmsHTK.get3phIt(); triPhones.hasNext();) {
            ...
            {
            	// 2ph are transformed into 3ph with SIL context, as it looks like
            	// S4 do not support 2ph ?!
            ...
            // TODO: pour le moment, je ne sais pas si les HMMs sont aux extremites des mots ou pas !
            String position = "i";
            int tmat = hmm.trIdx;
    ...
    class HTKstruct {
        ...
        public void load(String nom) {
            ...
            hmmsHTK = new HMMSet();
            hmmsHTK.loadHTK(nom);
            // TODO: avec sphinx4, la tiedlist est inutile ??
            //hmmsHTK.loadTiedList(nom+".tiedlist");
        ...
        public Pool<float[]> htkMeans(String path) {
            Pool<float[]> pool = new Pool<float[]>(path);
            // je suppose qu'il s'agit du nb d'etats partages ?!
            ...
            for (int i = 0; i < numStates; i++) {
               GMMDiag gmm = hmmsHTK.gmms.get(i);
                for (int j = 0; j < numGaussiansPerState; j++) {
                    ...
                    // the order of the means is the order in the HMMSet.gmms vector
                    // which is the order of appearance in the MMF file
                    pool.put(id, density);
                }
            }
            return pool;
        ...
        public Pool<float[]> htkVars(String path, float floor) {
            ...
            for (int i = 0; i < numStates; i++) {
                ...
                for (int j = 0; j < numGaussiansPerState; j++) {
                    ...
                    int id = i * numGaussiansPerState + j;
                    // the order of the vars is the order in the HMMSet.gmms vector
                    // which is the order of appearance in the MMF file
                    pool.put(id, vars);
                }
            }
        ...
        public Pool<float[]> htkWeights(String path, float floor) {
            ...
            for (int i = 0; i < numStates; i++) {
                ...
                // the order of the weights is the order in the HMMSet.gmms vector
                // which is the order of appearance in the MMF file
        ...
        public Pool<float[][]> htkTrans(String path) {
            ...
            /*
             * the order of the transitions is first, the order of the trans macro,
             * and then the order of the trans in the HMMs... in other words, this is
             * simply the order of the trans in the MMF !
             */
            ...
            for (int l=0; l < numMatrices; l++) {
                ...
                if (hmm.trans!=null) {
                    ...
                    // the index in the pool is the same as the index in the macros !!
