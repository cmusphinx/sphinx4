...
;
	...
	private Pool<Senone> createSenonePool(float distFloor, float varianceFloor) {
		...
		// TODO: remove this variable to allow different number of states per HMM
	...
	protected void loadHMMPool(boolean useCDUnits, HTKstruct htkmods,
		...
		/**
		 * This version loads the phones from the .MMF But contrary to S4, HTK do
		 * not produce reliable monophones, and some monophones may be missing ! An option
		 * might be to load 1ph trained independently from the 3ph but this file
		 * may not be always available. An easy fallback solution for now is to
		 * tie all the monophone to the first 3ph with the same base name found...
		 */
		if (!tie1ph) {
		...
		for (Iterator<SingleHMM> triPhones = htkmods.hmmsHTK.get3phIt(); triPhones
			...
			{
				// diphones are transformed into triphones with SIL context, as it looks
			...
			// TODO: For this moment we don't know if the HMMS are in the end or not
			String position = "i";
			int tmat = hmm.trIdx;
