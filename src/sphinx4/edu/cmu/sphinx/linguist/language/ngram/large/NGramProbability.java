...
class NGramProbability {
   ...
   *
   * @param which which follower of the first word is this NGram
   * @param wordID the ID of the Nth word in a NGram
   * @param probabilityID the index into the probability array
   * @param backoffID the index into the backoff probability array
   * @param firstNPlus1GramEntry the first N+1Gram entry
   */
  public NGramProbability(int which, int wordID, int probabilityID, int backoffID, int firstNPlus1GramEntry) {
    this.which = which;
    this.wordID = wordID;
