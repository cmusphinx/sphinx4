 ...
 * SpeechMarker}. Such a sequence of Data for an utterance should look like one of the following two:
 * <p/>
 * <b>Case 1: Only one speech region</b> <p>In the first case, the data stream has only one speech region: <p><img
 ...
 * <p/>
 * We will use the example of a data stream with two speech regions to illustrate the case of a data stream with
 * multiple speech regions: <p><img src="doc-files/two-regions.gif"> <br><i>Figure 3: A data stream with two speech
 * regions.</i>
 * <p/>
 * This case is more complicated than one speech region. The property <b>mergeSpeechSegments</b> is very important in
 * controlling the behavior of this filter. This property determines whether individual speech regions (and the
...
public class NonSpeechDataFilter extends BaseDataProcessor {
    ...
    /**
     * The Sphinx Property that controls whether to merge discontiguous speech segments (and the non-speech segments
     * between them) in an utterance into one big segment (true), or to treat the individual speech segments as
    ...
    /**
     * Controls whether to merge multiple speech segments within an Utterance to one big speech segment, with the
     * boundaries being the start of the first speech segment, and the end of the last speech segment.
     */
