 ...
 * <p/>
 * The acoustic model should be packaged in a JAR file. The dictionary and language model files are not required to be
 * in the package. You can specify their locations separately. A text file called "model.props" and the data files that
 * make up the acoustic model are required. The model.props file is a file of key-value pairs, loadable as a Java
 * Properties file. It should minimally contain the following properties: <ul> <li><b>dataLocation</b> - this specifies
 * the directory where the actual model data files are, <i>relative to the model implementation class</i></li>
 * <li><b>modelDefinition</b> - this specifies the location where the model definition file is, <i>relative to the model
 * implementation class</i></li> </ul> The actual model data files are named "means", "variances",
 * "transition_matrices", "mixture_weights" for binary versions, or prepended with ".ascii" for the ASCII versions.
 * </p>
 * <p/>
 * As an example, lets look at the Wall Street Journal acoustic model JAR file, which is located at the
 * <code>sphinx4/lib</code> directory. If you run <code>"jar tvf lib/WSJ_8gau_13dCep_16k_40mel_130Hz_6800Hz.jar"</code>,
 ...
 * the WSJ_clean_13dCep_16k_40mel_130Hz_6800Hz.class): </p>
 * <pre>
 ...
 * <p/>
 * <p/>
 * Note that although most of the properties of this class are already defined in the model.props file, it is still
 * possible (but not recommended) to override those values by specifying them in the configuration file. </p>
 */
