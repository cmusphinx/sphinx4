...
/**
 * Combines the various unit feature stream scores in a CombineToken. This SameTimeScoreCombiner assumes that there can
 * be more than one token from each feature stream. It will take the highest scoring token from each stream <b>at each
...
public class SameTimeScoreCombiner implements ScoreCombiner {
     ...
     *
     * @param timeDifference the maximum difference in time between tokens for them to be considered approximately the
     *                       'same time'
     */
