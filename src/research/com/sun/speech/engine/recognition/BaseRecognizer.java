 ...
 * <p/>
 * This class is useful by itself for debugging, e.g. you can load grammars and simulate a recognizer recognizing some
 * text, etc.
 * <p/>
 * <p/>
 * Actual JSAPI recognizer implementations might want to extend or modify this implementation.
 * <p/>
 * <p/>
...
// Begin Constructors
     ...
     * Create a new Recognizer in the DEALLOCATED state.
     *
     * @param reloadAll set to true if recognizer cannot handle partial grammar loading.  Default = false.
     */
...
// Begin overridden Engine Methods
    ...
    /** Return an object that provides management of the audio input or output of the Engine. From javax.speech.Engine. */
...
// Begin Recognizer Methods
    ...
    /**
     * Create a new RuleGrammar with the given name. From javax.speech.recognition.Recognizer.
     *
    ...
    /**
     * NOT IMPLEMENTED YET. We use a JavaCC generated parser for reading jsgf file. It does not support reading from
     * java.io.Reader yet - once it does we will implement this method.  The current implementation assumes the Reader
     * can be converted into an ASCII input stream. From javax.speech.recognition.Recognizer.
     *
    ...
    /**
     * Load a RuleGrammar and its imported grammars from a URL containing JSGF text. From
    ...
    /** Get a list of loaded or defined RuleGrammars known to the Recognizer. From javax.speech.recognition.Recognizer. */
    ...
    /**
     * Delete a RuleGrammar from the Recognizer. From javax.speech.recognition.Recognizer.
     *
    ...
    /**
     * Get the DicationGrammar for this Recognizer. Always returns null.  Should be overridden by recognizers that
     * support dictation.
     */
    ...
    /** Temporarily suspend recognition while the application updates grammars prior to a call to commitChanges. */
    ...
    /**
     * NOT IMPLEMENTED YET. If the Recognizer is in the PROCESSING state, force the Recognizer to immediately complete
     * processing of that result by finalizing it. From javax.speech.recognition.Recognizer.
     *
    ...
    /**
     * Request notification of Result events from the Recognizer. From javax.speech.recognition.Recognizer.
     *
    ...
    /** Get the RecognizerProperties of this Recognizer. From javax.speech.recognition.Recognizer. */
    ...
    /**
     * NOT IMPLEMENTED YET. Get the object that manages the speakers of the Recognizer. From
     * javax.speech.recognition.Recognizer.
     */
    ...
    /**
     * Create a new gramar by reading in a grammar stored in a vendor-specific format.  Since BaseGrammar is
     * serializable we just use read/write object to store/restore grammars. From javax.speech.recognition.Recognizer.
     */
    ...
    /**
     * Create a new grammar by reading in a grammar stored in a vendor-specific format.  Since BaseGrammar is
     * serializable we just use read/write object to store/restore grammars. From javax.speech.recognition.Recognizer.
     */
    ...
    /**
     * Read a Result from a stream in a vendor-specific format. Since BaseResult is serializable we just use read/write
     * object to store/restore grammars. From javax.speech.recognition.Recognizer.
     */
    ...
    /**
     * Store a Result to a stream in a vendor-specific format. Since BaseResult is serializable we just use read/write
     * object to store/restore grammars. From javax.speech.recognition.Recognizer.
     */
...
// NON-JSAPI METHODS
    ...
    /**
     * NOT JSAPI. In the mean time since the above method is not implemented we support loading JSGF grammars from an
     * InputStream which is non-standard
     */
    ...
    /** Let listeners know the recognizer recognized/finalized/accepted result.  NOT JSAPI. */
...
// Begin utility methods for sending RecognizerEvents.
    ...
    /**
     * Utility function to generate CHANGES_COMMITTED event and post it to the event queue.  Eventually
     * fireChangesCommitted will be called by dispatchSpeechEvent as a result of this action.
     */
    ...
    /** Utility function to send a CHANGES_COMMITTED event to all engine listeners. */
    ...
    /**
     * Utility function to generate FOCUS_GAINED event and post it to the event queue.  Eventually fireFocusGained will
     * be called by dispatchSpeechEvent as a result of this action.
     */
    ...
    /**
     * Utility function to generate FOCUS_LOST event and post it to the event queue.  Eventually fireFocusLost will be
    ...
    /**
     * Utility function to generate RECOGNIZER_PROCESSING event and post it to the event queue.  Eventually
     * fireRecognizerProcessing will be called by dispatchSpeechEvent as a result of this action.
     */
    ...
    /**
     * Utility function to generate RECOGNIZER_SUSPENDED event and post it to the event queue.  Eventually
     * fireRecognizerSuspended will be called by dispatchSpeechEvent as a result of this action.
     */
...
//////////////////////
    ...
    /**
     * Cycle through each rule in each grammar calling the method changeRule for each method that had changed or is new.
     * Also make a list of all enabled rules.
     * <p/>
     * Nominally calling changeRule method will propogate the new rule to the underlying recognizer
     */
    ...
    /** Check each grammar and load and imported grammars that are not already loaded. */
    ...
    /**
     * Load grammars imported by a fullqualified Rule Token if they are not already loaded.
     *
     * @param r
     * @param g
     * @param context
    ...
    /** Determine if the Recognizer has any modal grammars.  This sets the global flag named hasModalGrammars. */
    ...
    /**
     * Determine if the given Grammar is active.  This is a combination of the enabled state and activation modes of the
     * Grammar as well as the current focus state of the recognizer.  NOT JSAPI.
     */
